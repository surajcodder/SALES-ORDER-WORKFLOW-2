sap.ui.define(
  ...
  function (BaseController) {
    ...
    return BaseController.extend("SalesOrderFlow.workflowuimodule.controller.App", {
      ...
      onBeforeRendering: function () {
        debugger
        // getAccessToken(
        //   'sb-Mahindra_sales-44f10b5ftrial-dev1!t369427',
        //   'b60ebdcc-3dbb-449e-b0f6-f32b50151396$mwrsB0Ik71GDhwRBFFCDHyFdKlw8B3AJ9WTI3zrTkUQ=',
        //   'https://44f10b5ftrial.authentication.us10.hana.ondemand.com/oauth/token',
        //   'pradeep.n@peolsolutions.com',
        //   'Pradeep@123'
        // );

        // debugger

        // function getAccessToken(clientId, clientSecret, authUrl, userId, userPassword) {
        //   debugger
        //   const requestData = {
        //     grant_type: 'password',
        //     client_id: clientId,
        ...
        //       console.error('Error on token generate:', xhr.responseText || error);
        //     }
        //   });
        // }
      ...
      onOpenPressed: async function (oEvent) {
        ...
        var oView = this.getView();
        var oData = oView.oPropagatedProperties.oModels.context.oData;
        var credentials = JSON.parse(oData.baseUrl); // Parse the JSON string into an object

        // Now you can access individual fields
        var clientId = credentials.clientid;
        var clientSecret = credentials.clientsecret;
        var xsuaaAuthUrl = credentials.xsuaaauthurl;
        ...
        // Regex pattern to match the "EnquiryFiles" part of the URL
        ...
        if (match) {
          ...
          accessToken = await getAccessToken(clientId, clientSecret, xsuaaAuthUrl, userId, userPassword);
          if (accessToken) {
            const pdfBlob = await fetchData(entityUrl, accessToken);
            if (pdfBlob) {
        ...
        async function fetchData(url, accessToken1) {
          return new Promise((resolve, reject) => {
            jQuery.ajax({
              url: url,
              method: "GET",
              headers: {
                'Authorization': 'Bearer ' + accessToken1,  // Include the token in headers
              },
              xhrFields: {
        ...
        function getAccessToken(clientId, clientSecret, authUrl, userId, userPassword) {
          ...
          return new Promise((resolve, reject) => {
            ...
            $.ajax({
              ...
              error: function (xhr, status, error) {
                console.error('Error on token generation:', xhr.responseText || error);
                reject(error);
              }
            });
          });
